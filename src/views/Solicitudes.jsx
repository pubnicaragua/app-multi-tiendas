...
function Solicitudes() {
  ...
  const handleConfirmation = async (storeId, isApproved) => {
    try {
      ...
      if (updateError) throw (updateError, alert(updateError))

      const { error: storeError } = await supabase.from("store").update({ is_active: true }).eq("id", storeId)

      if (storeError) throw (storeError, alert(updateError))
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    {/* Agregar sección de características */}
                    {solicitud.features.length > 0 && (
                      <>
                        <div className="pt-2 mt-2 border-t">
                          <h4 className="font-medium mb-2 text-sm">Características adicionales</h4>
                          {solicitud.features.map((feature) => (
                    ...
                    <div className="pt-2 mt-2 border-t">
                      ...
                      {solicitud.features.length > 0 && (
                        <div className="flex justify-between font-medium mt-1">
                          <span>Total Características:</span>
                          <span>${solicitud.features.reduce((sum, feature) => sum + feature.price, 0)}</span>
                        </div>
                      )}
                      <div className="flex justify-between font-medium text-blue-600 mt-2 pt-2 border-t">
                        <span>Total General:</span>
                        <span>
                          $
                          {solicitud.modules.reduce((sum, module) => sum + module.price, 0) +
                            solicitud.features.reduce((sum, feature) => sum + feature.price, 0)}
                        </span>
                      </div>
                    </div>
                  </div>
